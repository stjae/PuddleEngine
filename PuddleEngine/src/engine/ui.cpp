...
void UI::DrawDockSpace(Scene& scene, bool& init)
{
    ...
    if (ImGui::BeginMenuBar()) {
        ...
        vk::DescriptorSet playStatusIcon = scene.m_isPlaying ? m_stopIconDescriptorSet : m_playIconDescriptorSet;
        ImGui::PopItemFlag();
        if (ImGui::ImageButton("playStatusIcon", (ImTextureID&)playStatusIcon, { GetIconSize(buttonSize, GetButtonPadding(buttonSize, paddingRatio)), GetIconSize(buttonSize, GetButtonPadding(buttonSize, paddingRatio)) }, { 0, 0 }, { 1, 1 })) {
        ...
        if (ImGui::BeginPopupModal("New Scene", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            ImGui::InputTextWithHint("##SceneName", "Scene Name", sceneName.data(), sceneName.size());
            ImGui::PopItemWidth();
            ImGui::Separator();
...
void UI::DrawViewport(Scene& scene, Viewport& viewport)
{
    ...
    ImGui::Image((ImTextureID&)m_viewportImageDescriptorSet, ImVec2{ viewport.m_panelSize.x, viewport.m_panelSize.y });
...
void UI::DrawSceneAttribWindow(Scene& scene)
{
    ...
    if (ImGui::BeginTabItem("IBL")) {
        ...
        if (ImGui::ImageButton("plusIcon", (ImTextureID&)m_plusIconDescriptorSet, { GetIconSize(iblButtonSize, GetButtonPadding(iblButtonSize, 0.4f)), GetIconSize(iblButtonSize, GetButtonPadding(iblButtonSize, 0.4f)) }, ImVec2(0, 0), ImVec2(1, 1), ImVec4(0, 0, 0, 0), ImVec4(1, 1, 1, 1))) {
...
void UI::DrawResourceWindow(Scene& scene)
{
    ...
    if (ImGui::BeginTabItem("Model")) {
        ...
        if (ImGui::ImageButton("plusIcon", (ImTextureID&)m_plusIconDescriptorSet, { GetIconSize(resourceButtonSize, GetButtonPadding(resourceButtonSize, 0.4f)), GetIconSize(resourceButtonSize, GetButtonPadding(resourceButtonSize, 0.4f)) }, ImVec2(0, 0), ImVec2(1, 1), ImVec4(0, 0, 0, 0), ImVec4(1, 1, 1, 1))) {
        ...
        for (int i = 0; i < scene.m_meshes.size(); i++) {
            ImGui::PushID(i);
            ImGui::ImageButton("cubeIcon", (ImTextureID&)m_cubeIconDescriptorSet, { GetIconSize(resourceButtonSize, GetButtonPadding(resourceButtonSize, 0.4f)), GetIconSize(resourceButtonSize, GetButtonPadding(resourceButtonSize, 0.4f)) }, ImVec2(0, 0), ImVec2(1, 1), ImVec4(0, 0, 0, 0), ImVec4(1, 1, 1, 1));
...
void UI::DrawLightIcon(const Scene& scene, const Viewport& viewport)
{
    for (auto& light : scene.m_pointLight.m_UBOs) {
        ...
        offset = { offset.x / posZ, offset.y / posZ };
        if (posZ > 1.0f && scene.m_showLightIcon && !scene.IsPlaying())
            ImGui::GetWindowDrawList()->AddImage((ImTextureID&)m_lightIconDescriptorSet, viewport.m_panelPos + screenPos - offset, viewport.m_panelPos + screenPos + offset, ImVec2(0, 0), ImVec2(1, 1), IM_COL32_BLACK);
    }
}
    ...
    for (auto& camera : scene.m_cameras) {
        ...
        if (posZ > 1.0f && scene.m_showLightIcon && !scene.IsPlaying()) {
            if (scene.m_playCamera == camera.second)
                ImGui::GetWindowDrawList()->AddImage((ImTextureID&)m_cameraIconDescriptorSet, viewport.m_panelPos + screenPos - offset, viewport.m_panelPos + screenPos + offset, ImVec2(0, 0), ImVec2(1, 1), IM_COL32_BLACK);
            else
                ImGui::GetWindowDrawList()->AddImage((ImTextureID&)m_noCameraIconDescriptorSet, viewport.m_panelPos + screenPos - offset, viewport.m_panelPos + screenPos + offset, ImVec2(0, 0), ImVec2(1, 1), IM_COL32_BLACK);
        }
    }
}
